...
Trainer::Trainer(int t_id,int t_capacity):capacity(t_capacity),id(t_id)
{
    open = false;
}
...
{
    if (customer != nullptr && customersList.size()<capacity) {
...
Trainer::Trainer(Trainer&& other):capacity(other.getCapacity()),open(other.isOpen()),id(other.id)
{
    other.capacity = 0;
    other.clear();
}
...
void Trainer::copy(const Trainer& other)
{
    ...
    for (Customer* c : other.customersList) {
        Customer* new_cp = c;
        customersList.push_back(new_cp);
    }
